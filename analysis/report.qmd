...
# Print the table
nan_and_duplicates_summary <- check_data_quality(car_data)
...
splitted_data = split_data(car_data_clean, response_col = "safety", prop = 0.8)
model_metrics <- cross_validate_knn(train_x = splitted_data$train_x,
  train_y = splitted_data$train_y,
  k_values = seq(1, 21, 2))
best_row <- model_metrics[which.max(model_metrics$accuracy), ]
best_k <- best_row$k
best_cv_accuracy <- best_row$accuracy
final_model <- train_final_model(splitted_data$test_x, splitted_data$test_y, best_k)
predictions <- predict(final_model, splitted_data$test_x)
test_accuracy <- round(compute_test_accuracy(predictions, splitted_data$test_y), 2)
rand <- round(calculate_random_baseline(splitted_data$train_y), 2)

model_metrics
```

The final kNN model achieved `r test_accuracy`% accuracy. This is not a great accuracy, but it's better than random guessing, which would be `r rand`%.
