...
best_cv_accuracy <- round(max(cv_results$accuracy)*100, 2)
...
rand <- round((max(car_data_summary[car_data_summary$Variable == "safety", "Count"])/nrow(car_data))*100, 2)

# Convert encoded variables back to their categorical labels for plotting
...
This project analyzes the **Car Evaluation Dataset** from @UCI1988, which contains `r nrow(car_data)` rows with **six categorical attributes: buying price**, maintenance cost, number of doors, passenger capacity, luggage space, and safety rating. The dataset was clean, with no missing values or duplicates, and required **ordinal encoding** for machine learning analysis.

Exploratory Data Analysis (EDA) revealed balanced distributions across most features. However, the "class" variable was highly imbalanced, with "unacceptable" accounting for `r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 1, "Count"]` instances and "very good" only `r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 4, "Count"]` instances. The k-Nearest Neighbors (kNN) model was trained to predict **safety levels** using the other features, achieving an accuracy of `r test_accuracy`% with an optimal k of `r best_k`—an improvement over random guessing (`r rand`%) but highlighting the model's limited predictive power.
...
Although the dataset comes with predefined columns, it does not include column names when we read in the CSV file. We manually assign meaningful column names based on the UCI dataset documentation as shown in @tbl-col.
...
# Create data frame for missing values display
df <- data.frame(
...
As shown in @tbl-nan, there are no missing values or duplicate rows in our dataset.

### **Data Types**

Since k-Nearest Neighbors (kNN) is a distance-based algorithm, it requires numerical input for feature comparisons. However, our dataset consists of categorical variables (all factors in R).
...
@tbl-enc shows the transformed dataset after ordinal encoding. Here's the meaning of each numerical value:
...
# Print summary of occurrences
print(car_data_summary, n = Inf)
```

@tbl-cnt provides a clearer picture of the dataset's distribution across categorical variables. The `buying`, `maint`, and `doors` features all have an even distribution, with each level appearing `r car_data_summary[1, "Count"]` times, suggesting a balanced dataset for these features. The `safety`, `lug_boot` and `persons` features are also evenly distributed across its three levels, with `r car_data_summary[car_data_summary$Variable == "safety" & car_data_summary$Value == 1, "Count"]` occurrences each, ensuring no significant class imbalance in this variable. However, the `class` is highly imbalanced, with "unacc" (encoded as 1) making up the majority (`r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 1, "Count"]` occurrences), while "vgood" (encoded as 4) is rare (only `r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 4, "Count"]` occurrences). Since we are not using the `class` variable in our analysis, this is not much of a concern.
...
# Cross-validation results (hidden from output)
...
Since we are using multiple input variables for our kNN classification, it wouldn't be possible to graph it out in 2D. We will use a **confusion matrix** as our visualization.
...
ggplot(conf_matrix_df, aes(x = Predicted_Label, y = True_Label, fill = Count)) +
  ...
  theme_minimal() + larger_theme  # Apply a clean, minimal theme
```

@fig-conf shows the heatmap visualization of our confusion matrix for the kNN classification model.

## **Discussion**

### **What We Found**

The analysis started with a car evaluation dataset lacking column names, which were manually assigned as "buying," "maint," "doors," "persons," "lug_boot," "safety," and "class" based on UCI documentation. The dataset, comprising `r nrow(car_data)` rows, was clean with no missing values or duplicates. All variables were categorical and ordinal, necessitating ordinal encoding into numerical values for k-Nearest Neighbors (kNN) analysis. Summary statistics revealed balanced distributions across most features (e.g., "buying," "maint," "doors," "lug_boot," "safety," and "persons"), with each category appearing `r car_data_summary[1, "Count"]` or `r car_data_summary[car_data_summary$Variable == "safety" & car_data_summary$Value == 1, "Count"]` times. However, the "class" variable was highly imbalanced, with "unacc" dominating (`r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 1, "Count"]` instances) and "vgood" rare (`r car_data_summary[car_data_summary$Variable == "class" & car_data_summary$Value == 4, "Count"]` instances), though this was irrelevant as "class" was not used in the kNN analysis.

Exploratory Data Analysis (EDA) using histograms as shown in @fig-buy, @fig-num, and @fig-main revealed that "safety" levels (low, medium, high) were evenly distributed across "buying," "persons," and "maint," indicating no single feature strongly distinguished safety levels independently. The kNN model, trained to predict "safety" using other features, achieved a final accuracy of `r test_accuracy`% with an optimal k of `r best_k`, surpassing random guessing (`r rand`% for three classes) but suggesting limited predictive power. A confusion matrix heatmap visualized in @fig-conf shows the model's performance, highlighting prediction accuracies and errors across safety levels.
